...
public class NukGui implements MouseListener, ActionListener {
    ...
    public void flipOver ( int serialNum, Color player ) {
        ...
        MasuData[] neighbors = board.neighbors(masu_row, masu_col);

        // 八方向すべてについて検討する。
        for (int i = 0; i < 8; i++) {
            // 各方向について敵石の数をカウントする。
            int enemyPoint = gameHelper.countEnemy( i, neighbors[i], player, 0);
            System.out.println("方向:" + i + " 敵:" + enemyPoint );
            if (enemyPoint > 0) {
                // int index = serialNum + direction[i].getDir();
                System.out.println("敵石の位置:" + neighbors[i].getNum() );
                System.exit(1);
            	MasuData nextMasu = board.neighbor( i, neighbors[i] );
